...
/**
 * Fügt benutzerdefinierte Admin-Styles hinzu
 */
function trendy_admin_styles() {
    wp_enqueue_style('trendy-admin-styles', get_template_directory_uri() . '/assets/css/admin.css');
}
add_action('admin_enqueue_scripts', 'trendy_admin_styles');
...
/**
 * Bild ID im Anhangsmodal anzeigen und kopieren
 */
function trendy_show_image_id_in_attachment_modal($form_fields, $post) {
    $unique_id = 'image-id-' . $post->ID;
    
    $html = '<div class="image-id-copy-container">';
    $html .= '<input type="text" id="' . $unique_id . '" readonly value="' . $post->ID . '" class="widefat" style="width:70%; display:inline-block;" />';
    $html .= '<button type="button" class="button button-small copy-image-id" data-clipboard-target="#' . $unique_id . '" style="margin-left:5px;">Kopieren</button>';
    $html .= '<span class="copy-feedback" style="display:none; color:green; margin-left:5px;">✓ Kopiert</span>';
    $html .= '</div>';
    
    $html .= "<script>
        jQuery(document).ready(function($) {
            $('.copy-image-id').click(function() {
                var textField = $(this).data('clipboard-target');
                $(textField).select();
                document.execCommand('copy');
                
                // Show feedback
                $(this).next('.copy-feedback').fadeIn().delay(1000).fadeOut();
    ...
    $form_fields['image_id'] = array(
        'label' => __('Bild ID', 'envo-shopper'),
        'input' => 'html',
        'html' => $html,
        'helps' => __('Klicken Sie auf "Kopieren", um die ID in die Zwischenablage zu kopieren', 'envo-shopper'),
    );
    
    return $form_fields;
}
add_filter('attachment_fields_to_edit', 'trendy_show_image_id_in_attachment_modal', 10, 2);
